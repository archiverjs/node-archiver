...
Archiver.prototype._updateQueueTaskWithStats = function(task, stats) {
  if (stats.isFile()) {
    ...
    var linkAbsPath = path.resolve(linkPath);

    task.data.type = 'symlink';
    console.log(['filepath', task.filepath]);
    console.log(['readlink', fs.readlinkSync(task.filepath)]);
    console.log(['linkAbsPath', linkAbsPath]);
    console.log(['relative', path.relative(path.dirname(task.filepath), linkAbsPath)]);
    task.data.linkname = path.relative(path.dirname(task.filepath), linkAbsPath);
    task.data.sourceType = 'buffer';
    task.source = new Buffer(0);
  } else {
