...
Archiver.prototype.symlink = function(filepath, target) {
    if (this._state.finalize || this._state.aborted) {
        this.emit('error', new Error('symlink: queue closed'));
        return this;
    }

    if (!this._moduleSupports('symlink')) {
        this.emit('error', new Error('symlink: feature not currently supported by this module'));
        return this;
    }

    if (typeof filepath !== 'string' || filepath.length === 0) {
        this.emit('error', new Error('symlink: filepath must be a non-empty string value'));
        return this;
    }

    if (typeof target !== 'string' || target.length === 0) {
        this.emit('error', new Error('symlink: target must be a non-empty string value'));
        return this;
    }
 ...
 * @property {Object} fs
 * @property {Number} fs.totalBytes Number of bytes based on fs.Stats of appended entries.
 * @property {Number} fs.processedBytes Number of bytes that hae been processed.
 */