...
Archiver.prototype._updateQueueTaskWithStats = function(task, stats) {
  if (stats.isFile()) {
    ...
    console.log(['filepath', task.filepath]);
    console.log(['readlink', fs.readlinkSync(task.filepath)]);
    console.log(['relative', path.relative(task.filepath, fs.readlinkSync(task.filepath))]);
    console.log(['relative', path.relative(fs.readlinkSync(task.filepath), task.filepath)]);
    task.data.linkname = path.relative(task.filepath, fs.readlinkSync(task.filepath));
    task.data.sourceType = 'buffer';
    task.source = new Buffer(0);
  } else {
