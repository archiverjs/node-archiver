...
function HeaderZipFile() {
  ...
  this.fields = [
    ...
    {field: 'versionNeededToExtract', len: 2, type: 'UInt16LE'},
...
HeaderZipFile.prototype.toBuffer = function(data) {
  if (data.extraField && data.extraField.length > 0) {
    data.extraFieldLength = data.extraField.length;
  }

  return HeaderZip.prototype.toBuffer.call(this, data);
};
...
function HeaderZipCentralDirectory() {
  ...
  this.fields = [
    ...
    {field: 'versionMadeBy', len: 2, type: 'UInt16LE'},
    {field: 'versionNeededToExtract', len: 2, type: 'UInt16LE'},
    {field: 'flags', len: 2, type: 'UInt16LE'},
    {field: 'compressionMethod', len: 2, type: 'UInt16LE'},
    {field: 'lastModifiedDate', len: 4, type: 'UInt32LE'},
...
HeaderZipCentralDirectory.prototype.toBuffer = function(data) {
...
HeaderZipCentralFooter.prototype.toBuffer = function(data) {
  if (data.comment && (data.comment.length > 0)) {
    data.commentLength = data.comment.length;
  }
