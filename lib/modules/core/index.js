...
Archiver.prototype.bulk = function(mappings) {
  ...
  files.forEach(function(file){
    ...
    file.src.forEach(function(filepath) {
      ...
      data.type = data.name.slice(-1) === '/' ? 'directory' : 'file';

      if (data.type === 'directory' && !self._moduleSupports('directory')) {
        return;
      } else if (data.type === 'file' && !util.file.exists(filepath)) {
        return;
      }

      data.sourcePath = filepath;
      data.sourceType = data.type === 'file' ? 'stream' : 'buffer';

      self._queue.add({
        data: data,
        source: data.type === 'file' ? util.lazyReadStream(filepath) : new Buffer(0)
      });
    });
  });
