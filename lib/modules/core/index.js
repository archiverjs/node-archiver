...
Archiver.prototype.directory = function(dirpath, callback) {
  var self = this;
  ...
  if (util.file.isDir(dirpath)) {
    util.file.recurse(dirpath, function(filepath, rootdir, subdir, filename) {
      var data = {
        name: subdir ? subdir + '/' + filename : filename
      };

      self.append(util.lazyReadStream(filepath), data);
    });
  } else {
    callback(new Error('Invalid directory: ' + dirpath));
  }
