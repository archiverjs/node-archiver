...
Archiver.prototype._normalizeFileData = function(data, stat) {
  stat = stat || false;
  ...
  if (typeof data.mode === 'number') {
    ...
    data.mode = stat.mode & 0777;
  } else {
    data.mode = isDir ? 0755 : 0644;
  }

  if (stat && data.date === null) {
...
Archiver.prototype.bulk = function(mappings) {
  ...
  files.forEach(function(file){
    ...
    file.src.forEach(function(filepath) {
      ...
      self._queue.add({
        data: self._normalizeFileData(data, stat),
        source: source
      });
    });
  });
...
Archiver.prototype.file = function(filepath, data) {
  ...
  data = this._normalizeFileData(data, stat);
